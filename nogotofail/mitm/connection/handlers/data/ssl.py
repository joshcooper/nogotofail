...
class InsecureCipherDetectionHandler(DataHandler):
    ...
    def on_ssl(self, client_hello):
        ...
        # des_ciphers = [str(c) for c in client_hello.ciphers if "_DES" in str(c)]
        # if des_ciphers:
        #     self._handle_bad_ciphers(des_ciphers,
        #         "Client enabled DES TLS/SSL cipher suites %s" %
        #         (", ".join(des_ciphers)))

        # tripledes_ciphers = [str(c) for c in client_hello.ciphers if "3DES" in str(c)]
        # if tripledes_ciphers:
        #     self._handle_bad_ciphers(tripledes_ciphers,
        #         "Client enabled 3DES TLS/SSL cipher suites %s" %
        #         (", ".join(tripledes_ciphers)))
